...
var (
	...
	titleBAST         = utils.GetEnv("BAST_TITLE_CONTENT")
	contentTextBAST   = utils.GetEnv("BAST_CONTENT_TEXT")
	// buttonTextBAST    = utils.GetEnv("BUTTON_TEXT_RESET")
	...
	// urlBAST           = utils.GetEnv("URL_RESET_PASSWORD")
	linkPicture = utils.GetEnv("LOGO_URL_RESET")

	receiver = strings.Split(utils.GetEnv("BAST_EMAIL_RECEIVER"), ",")
)

func EmailSender(file bytes.Buffer) error {
	if len(receiver) == 0 {
		return errors.New("email retriever has not been set")
	}

	user := emails
	password := password

	subject := "New BAST Document from 001M Plant"
	body := `<!doctype html>
	<html lang="en-US">
 ...
 <body class=""
    		    ...
    		    <tbody>
    		        <tr>
    		            <td width="100%" align="center" style="padding-top:10px;">
										<img class="center" src="` + linkPicture + `" width="100">
    		            </td>
    		        </tr>
    		    </tbody>
    		</table>
	...
	if err != nil {
		logger.Errorw("Failed to connect to the smtp client", "error", err)
	}
	...
	// attachment
	emails.Attach(&mail.File{Data: file.Bytes(), Name: "bast.pdf"})

	// always check error after send
	if emails.Error != nil {
		logger.Errorw("Failed to create email", "error", emails.Error)
	}
