...
var (
	emailSender    = utils.GetEnv("EMAIL_SENDER")
	emailReceiver  = utils.GetEnv("EMAIL_RECEIVER")
	sendgridApiKey = utils.GetEnv("SENDGRID_API_KEY")
	urlResetLocal  = utils.GetEnv("URL_RESET_PASSWORD_LOCAL")
)

func EmailSender2(email, token string) error {

	var encodedString = base64.StdEncoding.EncodeToString([]byte(token))
	token = encodedString

	url := urlReset + "?email=" + email + "&token=" + token
	from := mail.NewEmail("CS Iqbal", emailSender)
	subject := "Reset Password #" + strings.ToUpper(randstr.String(10))
	to := mail.NewEmail("Talent Iqbal", emailReceiver)
	plainTextContent := "Ini Reset Password"
    		    ...
    		    <tbody>
    		        <tr>
    		            <td width="100%" align="center" style="padding-top:10px;">
										<img alt="linkPicture" class="center" src="` + linkPicture + `" width="200" height="66" >
    		            </td>
    		        </tr>
    		    </tbody>
    		</table>
